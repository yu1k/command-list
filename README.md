...
ファイルをコピーすることができるコマンド

```
$ cp -r [コピー元ディレクトリ] [コピー後に配置したいディレクトリ]
```

ディレクトリごとコピーできる

```
$ cp -a [コピー元ディレクトリ] [コピー後に配置したいディレクトリ]
```

パーミッション情報などを保持したままディレクトリをコピーできる
...
パケットサイズを指定できる。実際に送信されるパケットサイズは指定したサイズに8バイト足したパケットが送信される。
...
- オプション
  - a
    - BGPのAS番号を表示。
  - P
    - 使用するプロトコルを指定する。UDP, TCPを指定できるのを確認しました。
...
aptなどでiperf3をインストールし、使用する。

サーバ側は `5201/tcp` ポートを解放。
...
```
$ iperf3 -c [サーバのIPアドレスやホストを指定]
```

クライアントモードで実行する。

```
$ iperf3 -c [サーバのIPアドレスやホストを指定] -t [実行したい時間を指定]
```

クライアントモードで実行する。実行する時間を指定できる。
